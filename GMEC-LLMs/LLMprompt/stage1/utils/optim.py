...
def get_optim_for_train(__C, model, new_params_name='proj'):
    # optimizer for finetuning warmup
    # change the projmec to mymec
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            if 'mymec' in name or 'proj' in name:
                params2[0]['params'].append(param)
            else:
                params2[1]['params'].append(param)
...
def get_optim_for_train2(__C, model, new_params_name='projmec'):
    optim_class2 = eval('Optim.' + __C.OPT)
    params2 = [
        {'params': [], 'lr': __C.LR_BASE, '_grad_norm_clip': __C.GRAD_NORM_CLIP},
        {'params': [], 'lr': __C.LR_BASE * __C.BERT_LR_MULT, '_grad_norm_clip': __C.GRAD_NORM_CLIP},
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            if 'proj' in name or 'classification_head' in name:
                params2[0]['params'].append(param)
            else:
                params2[1]['params'].append(param)
...
def get_optim_for_train3(__C, model,training_steps):
    optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()),
                    lr=1e-5, weight_decay=1e-6)
    scheduler = transformers.get_cosine_schedule_with_warmup(optimizer,
                                                num_warmup_steps=0.01*training_steps,
                                                num_training_steps=training_steps) # 0.01
    return OptimizerWrapper(
        [optimizer],
        scheduler=scheduler,
        warmup_schd_steps=None,
        decay_schd_step_list=None,
        decay_rate=None,
        change_optim_step_list=None        
    )