...
class Cfgs(PATH):
    ...
    def __init__(self, args):
        ...
        self.TASK = getattr(args, 'TASK', 'mec')
        assert self.TASK in ['mec']

        self.RUN_MODE = getattr(args, 'RUN_MODE', 'finetune')
        assert self.RUN_MODE in ['pretrain', 'finetune',
                                 'finetune_test','train','train_test', 'heuristics', 'prompt']

        if self.RUN_MODE == 'pretrain':
            self.DATA_TAG = 'v2'  # used to config answer dict
            self.DATA_MODE = 'pretrain'
        elif self.RUN_MODE == 'train':
            self.DATA_TAG = self.TASK.split(
                '_')[0]
            self.DATA_MODE = 'train'
        else:
            self.DATA_TAG = self.TASK.split(
                '_')[0]  # used to config answer dict
    ...
    def TEST_SPLITS(self):
        ...
        return TASK_TO_SPLIT[self.TASK][self.DATA_MODE]['test_split']
    @property
    def DEV_SPLITS(self):
      
        return TASK_TO_SPLIT[self.TASK][self.DATA_MODE]['dev_split']

    @property
    def FEATURE_SPLIT(self):
        FEATURE_SPLIT = []
        for split in self.TRAIN_SPLITS + self.TEST_SPLITS:
    ...
    def EVAL_QUESTION_PATH(self):
        if not self.EVAL_NOW:
            return []
        return self.QUESTION_PATH[self.TEST_SPLITS[0]]

    @property
    def EVAL_ANSWER_PATH(self):
        if not self.EVAL_NOW:
            return []
        return self.ANSWER_PATH[self.TEST_SPLITS[0]]

    @property
    def get_img_feature_path(self):
        return ''